    ...
    <div class="main-content">
      <div class="col-md-12">
        <app-data-table
          ...
          [emptyMessage]="'No unconfirmed transactions in the mempool.'"
...
export class UnconfirmedComponent implements OnInit {
  ...
  loadTransactions() {
    ...
    this.apiService.get<{ unconfirmedTransactions: UnconfirmedTransaction[] }>('getUnconfirmedTransactions', {
      firstIndex: firstIndex,
      lastIndex: lastIndex
    }).subscribe({
      next: (response) => {
        this.transactions = response.unconfirmedTransactions || [];
        this.loading = false;
        this.isReloading = false;
        this.updatePagination();
      },
      error: (error) => {
        console.error('Error loading unconfirmed transactions:', error);
        this.loading = false;
        this.isReloading = false;
      }
    });
